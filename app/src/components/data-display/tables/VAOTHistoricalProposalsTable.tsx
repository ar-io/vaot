...
import { useVAOTHistory } from '@/hooks/useVAOTHistory';
...
function VAOTHistoricalProposalsTable({ vaotId }: { vaotId?: string }) {
  ...
  const { allMessages } = useVAOTHistory();
  
  const acceptedProposalNodes = useMemo(()=> {
    if (allMessages && allMessages.length) {
      return allMessages.filter(node=> !!node.tags.find(t => t.name === 'Action' && t.value === 'Proposal-Accepted-Notice'))
    }
    return []
  },[allMessages])
